...
function updateModelRestrictions() {
  ...
  if (isOSeries) {
    ...
    modelWarning.textContent =
      "⚠️ This model only supports temperature=1 and topP=1";
    ...
    modelWarning.textContent =
      "⚠️ This model only supports temperature=1 and does not support topP parameter";
    modelWarning.style.display = "block";
...
async function loadConfig() {
  ...
  try {
    ...
    if (data.success) {
      ...
      // Set chat prompt if present
      if (chatPromptEl) {
        chatPromptEl.value = config.chatPrompt || "";
        console.log(
          "Loaded chatPrompt length:",
          (config.chatPrompt || "").length
        );
      }
...
async function saveConfig(event) {
  ...
  // Include single chat prompt (optional)
  if (chatPromptEl) {
    config.chatPrompt = chatPromptEl.value.trim();
  }
...
async function testConfig() {
  ...
  try {
    showAlert(
      "🧪 Testing configuration... This may take a few seconds.",
      "info"
    );
    ...
    if (data.success) {
      ...
      successMsg += `• Streaming: ${
        data.config.streaming ? "Enabled" : "Disabled"
      }\n`;
